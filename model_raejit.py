...
class JiTBlock(nn.Module):
    def __init__(self, hidden_size, num_heads, mlp_ratio=4.0, attn_drop=0.0, proj_drop=0.0, hidden_size_c=None):
        super().__init__()
...
class RAEJiT(nn.Module):
    ...
    def __init__(
        ...
        # linear embed
        self.pixel_embedder = BottleneckPatchEmbed(input_size, patch_size, in_channels, bottleneck_dim, hidden_size, bias=True)
        self.dino_embedder = BottleneckPatchEmbed(input_size // patch_size, 1, dino_in_channels, bottleneck_dim_dino, hidden_size, bias=True)
        ...
        if self.dh_depth > 0:
            ...
            self.dh_blocks_pixel = nn.ModuleList([
                JiTBlock(dh_hidden_size, num_heads * dh_hidden_size // hidden_size , mlp_ratio=mlp_ratio, hidden_size_c=hidden_size) for _ in range(dh_depth)
            ])
            self.dh_blocks_dino = nn.ModuleList([
                JiTBlock(dh_hidden_size, num_heads * dh_hidden_size // hidden_size , mlp_ratio=mlp_ratio, hidden_size_c=hidden_size) for _ in range(dh_depth)
            ])

            self.final_layer_pixel = FinalLayer(dh_hidden_size, self.out_channels_pixel, hidden_size_c=hidden_size)
            self.final_layer_dino = FinalLayer(dh_hidden_size, self.out_channels_dino, hidden_size_c=hidden_size)
        else:
    ...
    def initialize_weights(self):
        ...
        pos_embed = get_2d_sincos_pos_embed(self.pos_embed.shape[-1], int(self.pixel_embedder.num_patches ** 0.5))
        ...
        if self.dh_depth > 0:
            ...
            # w1 = self.pixel_embedder_dh.proj.weight.data
            # nn.init.xavier_uniform_(w1.view([w1.shape[0], -1]))
            # nn.init.constant_(self.pixel_embedder_dh.proj.bias, 0)
            # w1 = self.dino_embedder_dh.proj.weight.data
            # nn.init.xavier_uniform_(w1.view([w1.shape[0], -1]))
            # nn.init.constant_(self.dino_embedder_dh.proj.bias, 0)

        # Zero-out output layers:
        for final_layer in [self.final_layer] if self.dh_depth == 0 else [self.final_layer_pixel, self.final_layer_dino]:
    ...
    def forward(self, x, t, y, m=None):
        ...
        t_emb_pixel = self.t_pixel_embedder(t[0])
        t_emb_dino = self.t_dino_embedder(t[1])
        ...
        if m is None:
            ...
            x = self.pixel_embedder(x_pixel) * (~m[0][...,None]) + self.dino_embedder(x_dino) * (~m[1][...,None])
        x += self.pos_embed
        ...
        if self.dh_depth == 0:
            output_pixel, output_dino = self.final_layer(x, c).split([self.out_channels_pixel, self.out_channels_dino], dim=-1)
        else:
        ...
        output = [output_pixel, output_dino]

        return output


def RAEJiT_S_16(**kwargs):
    return RAEJiT(depth=12, hidden_size=384, num_heads=6,
               bottleneck_dim=128, in_context_len=32, in_context_start=4, patch_size=16, **kwargs)


def RAEJiT_B_16(**kwargs):
    return RAEJiT(depth=12, hidden_size=768, num_heads=12,
               bottleneck_dim=128, in_context_len=32, in_context_start=4, patch_size=16, **kwargs)

def RAEJiT_BM_16(**kwargs):
    return RAEJiT(depth=8, hidden_size=768, num_heads=12,
               bottleneck_dim=128, in_context_len=32, in_context_start=4, patch_size=16, **kwargs)

def RAEJiT_B_32(**kwargs):
    return RAEJiT(depth=12, hidden_size=768, num_heads=12,
               bottleneck_dim=128, in_context_len=32, in_context_start=4, patch_size=32, **kwargs)

def RAEJiT_L_16(**kwargs):
    return RAEJiT(depth=24, hidden_size=1024, num_heads=16,
               bottleneck_dim=128, in_context_len=32, in_context_start=8, patch_size=16, **kwargs)

def RAEJiT_LM_16(**kwargs):
    return RAEJiT(depth=20, hidden_size=1024, num_heads=16,
               bottleneck_dim=128, in_context_len=32, in_context_start=8, patch_size=16, **kwargs)

def RAEJiT_L_32(**kwargs):
    return RAEJiT(depth=24, hidden_size=1024, num_heads=16,
               bottleneck_dim=128, in_context_len=32, in_context_start=8, patch_size=32, **kwargs)

def RAEJiT_H_16(**kwargs):
    return RAEJiT(depth=32, hidden_size=1280, num_heads=16,
               bottleneck_dim=256, in_context_len=32, in_context_start=10, patch_size=16, **kwargs)

def RAEJiT_H_32(**kwargs):
    return RAEJiT(depth=32, hidden_size=1280, num_heads=16,
               bottleneck_dim=256, in_context_len=32, in_context_start=10, patch_size=32, **kwargs)


RAEJiT_models = {
    'RAEJiT-S/16': RAEJiT_S_16,
    'RAEJiT-B/16': RAEJiT_B_16,
    'RAEJiT-BM/16': RAEJiT_BM_16,
    'RAEJiT-B/32': RAEJiT_B_32,
    'RAEJiT-L/16': RAEJiT_L_16,
    'RAEJiT-LM/16': RAEJiT_LM_16,
    'RAEJiT-L/32': RAEJiT_L_32,
    'RAEJiT-H/16': RAEJiT_H_16,
    'RAEJiT-H/32': RAEJiT_H_32,
}
