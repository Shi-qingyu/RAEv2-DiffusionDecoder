...
class DenoiserRAEJiT(nn.Module):
    def __init__(
        ...
        self.net = RAEJiT_models[args.model](
            ...
            dino_in_channels=self.dino_hidden_dim,
        )
    ...
    def get_generate_timesteps(self, labels, z):
        ...
        if self.sample_mode in {"eval_shifted", "dino_first_shifted_aligned", "shifted_independent_uniform"}:
            ...
            timesteps_pixel = torch.cat([torch.zeros(self.steps//2+1, device=device), torch.linspace(0.0, 1.0, self.steps//2+1, device=device)[1:]], dim=0).view(-1, *([1] * z[0].ndim)).expand(-1, bsz, -1, -1, -1)
            timesteps_dino = torch.cat([torch.linspace(0.0, self.dino_max_t, self.steps//2+1, device=device), torch.full((self.steps//2+1,), self.dino_max_t, device=device)[1:]], dim=0).view(-1, *([1] * z[1].ndim)).expand(-1, bsz, -1, -1, -1)
    ...
    def get_reconstruction_timesteps(self, labels, z):
        # During reconstruction, keep the DINO latent clean and denoise pixels.
        device = labels.device
        bsz = labels.size(0)
        timesteps_pixel = torch.linspace(0.0, 1.0, self.steps + 1, device=device)
        timesteps_pixel = timesteps_pixel * self.gen_shift_pixel / (
            1 + (self.gen_shift_pixel - 1) * timesteps_pixel
        )
        timesteps_dino = torch.ones(self.steps + 1, device=device)

        timesteps_pixel = timesteps_pixel.view(-1, *([1] * z[0].ndim)).expand(
            -1, bsz, -1, -1, -1
        )
        timesteps_dino = timesteps_dino.view(-1, *([1] * z[1].ndim)).expand(
            -1, bsz, -1, -1, -1
        )
        return list(zip(timesteps_pixel, timesteps_dino))
    ...
    def forward(self, x, labels):
        ...
        x_pixel = x
        x_dino = self._encode_dino(x * 0.5 + 0.5)
        x = [x_pixel, x_dino]
    ...
    def generate(self, labels):
        ...
        z_dino = self.noise_scale * torch.randn(
            bsz,
            self.dino_hidden_dim,
            self.dino_latent_size,
            self.dino_latent_size,
            device=device,
        )
        z = [z_pixel, z_dino]
    ...
    def reconstruction(self, images, labels):
        ...
        bsz = labels.size(0)
        z_pixel = self.noise_scale * torch.randn(bsz, 3, self.img_size, self.img_size, device=device)
        z_dino = self._encode_dino(images * 0.5 + 0.5)
        z = [z_pixel, z_dino]
        timesteps = self.get_reconstruction_timesteps(labels, z)
    ...
    def _forward_sample(self, z, t, labels):
        ...
        if self.guidance_method == "autoguidance":
            ...
            if self.ag_net is not None and t[1].flatten()[0].item() >= 0.999:
                x_uncond = self.ag_net(z, [t[i].flatten() for i in [0,1]], torch.full_like(labels, self.num_classes))
            else:
                x_uncond = self.net(z, [t[i].flatten() for i in [0,1]], torch.full_like(labels, self.num_classes))
        else:
            raise NotImplementedError(f"Unknown guidance_method: {self.guidance_method}")
